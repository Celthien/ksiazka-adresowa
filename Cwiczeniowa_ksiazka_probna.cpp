...
int logowanieUzytkownika (vector <lista_uzytkownikow> &zarejestrowaniUzytkownicy)
{
    ...
    cout <<"**Zeby wrocic do menu glownego, wprowadz zero.**"<<endl;
    cout <<"Login: ";
    cin >> login;
...
void zapisaniePlikuWlasciwego(vector <ksiazka_adresowa> &WpisWKsiazceAdresowej)
{
    ...
    string porownywanaLiniaPoEdycjach, porownywanaLiniaOryginalna;
    int dlugoscLiniiPoEdycjach, dlugoscLiniiOryginalnej;
    string idPoEdycjach, idOryginalne;
    ...
    if (plikGlowny.good())
    {
        ...
        {
            idOryginalne.clear();
            dlugoscLiniiOryginalnej = porownywanaLiniaOryginalna.length();

            for (int i=0; i<dlugoscLiniiOryginalnej; i++)
            {
                znak = porownywanaLiniaOryginalna[i];
                if (znak=='|') break;
                else idOryginalne+=znak;
            }
            idAdresata=atoi(idOryginalne.c_str());
            ...
            for (int p=0; p<WpisWKsiazceAdresowej.size(); p++)
            {
                ...
                {
                    plikTymczasowy << WpisWKsiazceAdresowej[p].pozycjaWpisu<<"|";
                    plikTymczasowy << WpisWKsiazceAdresowej[p].idUzytkownika<<"|";
                    plikTymczasowy << WpisWKsiazceAdresowej[p].imie<<"|";
                    plikTymczasowy << WpisWKsiazceAdresowej[p].nazwisko <<"|";
                    plikTymczasowy << WpisWKsiazceAdresowej[p].telefon <<"|";
                    plikTymczasowy << WpisWKsiazceAdresowej[p].email <<"|";
                    plikTymczasowy << WpisWKsiazceAdresowej[p].adres <<"|"<<endl;
                    break;
                }
                else weryfikacja++;
